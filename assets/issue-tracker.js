...
function parseFixedPhotos(fp) {
  fp = String(fp || '').trim();
  if (!fp) return [];
  return fp.split('|').map(function (u) { return u.trim(); }).filter(Boolean);
}
function joinFixedPhotos(urls) {
  return (urls || []).filter(Boolean).join('|');
}
function fixedPhotosBlockHtml(fixedPhoto) {
  var urls = parseFixedPhotos(fixedPhoto);
  if (!urls.length) return '<p style="color:var(--text-faint);">Not fixed yet</p>';
  return urls.map(function (u) {
    return '<img src="' + u + '" onclick="bigImg(this.src)" style="max-width:100%;margin-bottom:8px;border-radius:8px;display:block;">';
  }).join('');
}
