...
function patchIssueModalAssign(id) {
  ...
  if (h2) {
    h2.innerHTML = '<span style="color:var(--text-faint);font-weight:700;">#' + issueRef(r.num) + '</span> ' + r.issueType + tradeBadgeHtml(r);
  }
}
function assignIssue(id) {
  ...
  requestAnimationFrame(function () { buildTradeFilterBar(); });
  fetchJSONRetry({ action: ISSUE_CFG.actions.assign, id: id, assignedGroup: group, token: issueToken() || '' }, 1, 60000)
    .then(function (d) {
      if (d && d.ok === false) {
        if (empireAuthHandleInvalidSession_(d, issueSessionLogoutOpts())) return;
        throw new Error(d.message || d.error || 'Assign failed');
      }
      if (it && d && d.assignedGroup !== undefined) it.assignedGroup = d.assignedGroup;
      writeIssuesCacheAsync(allIssues);
    ...
    .catch(function (e) {
      if (it) it.assignedGroup = prevGroup;
      writeIssuesCacheAsync(allIssues);
...
function canBulkAssignIssues(){ return tradeGroups().length && ISSUE_CFG.actions.assign && PAGEPERMS.assign !== false; }
...
function assignSelectedIssues(){ var ids=Object.keys(selectedIssueIds); if(!ids.length){ alert('Select at least one issue first.'); return; } if(!canBulkAssignIssues()) return; var sel=document.getElementById('bulk-assign-group'); if(!sel) return; var group=sel.value; var label=tradeGroupLabel(group); uiConfirm('Assign <strong>'+ids.length+'</strong> issue(s) to <strong>'+label+'</strong>?').then(function(ok){ if(!ok) return; var prev={}; ids.forEach(function(id){ var it=allIssues.find(function(x){ return x.id===id; }); if(it){ prev[id]=it.assignedGroup||''; it.assignedGroup=group; } }); writeIssuesCacheAsync(allIssues); setBulkAssignBtnState('saving'); requestAnimationFrame(function(){ buildTradeFilterBar(); renderIssues(); }); fetchJSONRetry({ action:ISSUE_CFG.actions.assign, ids:ids, assignedGroup:group, token:issueToken()||'' }, 1, 90000).then(function(d){ if(d&&d.ok===false){ if(empireAuthHandleInvalidSession_(d, issueSessionLogoutOpts())) return; throw new Error(d.message||d.error||'Assign failed'); } if(d&&d.assignedGroup!==undefined){ ids.forEach(function(id){ var it=allIssues.find(function(x){ return x.id===id; }); if(it) it.assignedGroup=d.assignedGroup; }); } writeIssuesCacheAsync(allIssues); setBulkAssignBtnState('saved'); requestAnimationFrame(function(){ buildTradeFilterBar(); renderIssues(); }); }).catch(function(){ ids.forEach(function(id){ var it=allIssues.find(function(x){ return x.id===id; }); if(it&&prev.hasOwnProperty(id)) it.assignedGroup=prev[id]; }); writeIssuesCacheAsync(allIssues); setBulkAssignBtnState('error'); requestAnimationFrame(function(){ buildTradeFilterBar(); renderIssues(); }); }); }); }
function issueSelectToolbarHtml(){ var cnt=selectedIssueCount(); var bulk=canBulkAssignIssues(); var hint=bulk?'Tap cards to select several, then assign to a team or share on WhatsApp.':'Tap cards to pick several, then share on WhatsApp.'; if(!issueSelectMode) return '<div class="issue-select-bar"><span>'+hint+'</span><button type="button" onclick="toggleIssueSelectMode()" style="padding:8px 14px;font-size:12px;margin-left:auto;">Select issues</button></div>'; var h='<div class="issue-select-bar"><span><strong>'+cnt+'</strong> selected</span>'; if(bulk){ h+='<select id="bulk-assign-group" class="bulk-assign-select" onclick="event.stopPropagation()">'+bulkAssignTeamOptionsHtml()+'</select><button type="button" id="bulk-assign-btn" class="bulk-assign-btn" onclick="assignSelectedIssues()" '+(cnt?'':'disabled style="opacity:0.55;"')+'>Assign to team</button>'; } h+='<button type="button" onclick="shareSelectedWhatsApp()" style="background:#25D366;color:#fff;border:none;padding:8px 14px;font-size:12px;display:inline-flex;align-items:center;gap:6px;'+(cnt?'':'opacity:0.55;')+'" '+(cnt?'':'disabled')+'>'+whatsappIconHtml()+' Share on WhatsApp</button><button type="button" onclick="selectAllVisibleIssues()" style="padding:8px 14px;font-size:12px;">Select all</button><button type="button" onclick="clearIssueSelection()" style="padding:8px 14px;font-size:12px;">Clear</button><button type="button" onclick="toggleIssueSelectMode()" style="padding:8px 14px;font-size:12px;margin-left:auto;">Done</button></div>'; return h; }
...
function assignBoxHtml(r) {
  ...
  return '<div class="assign-box" onclick="event.stopPropagation()"><label>Assign to team</label><div class="assign-row"><select id="assign-group-' + r.id + '" onchange="setAssignBtnState(\'' + r.id + '\', \'idle\')">' + opts + '</select><button type="button" id="assign-btn-' + r.id + '" class="assign-save-btn" onclick="assignIssue(\'' + r.id + '\')">Save</button></div></div>';
}
