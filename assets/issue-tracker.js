...
function buildTradeFilterBar() {
  var host = document.getElementById('tradeFilterBar');
  if (!host || !tradeGroups().length) return;
  var cur = (document.getElementById('f-group') || {}).value || '';
  var open = allIssues.filter(function (r) { return r.status !== 'fixed'; });
  var un = open.filter(function (r) { return !r.assignedGroup; }).length;
  var list = [['', 'All (' + open.length + ')'], ['unassigned', 'Unassigned (' + un + ')']];
  tradeGroups().forEach(function (g) {
    var n = open.filter(function (r) { return r.assignedGroup === g.id; }).length;
    list.push([g.id, g.label + ' (' + n + ')']);
  });
  host.innerHTML = list.map(function (pair) {
    var p = pair[0], label = pair[1], sel = (cur === p);
    return '<button type="button" onclick="selectTradeFilter(\'' + p + '\')" style="background:' + (sel ? 'var(--accent2)' : 'var(--btn-bg)') + ';color:' + (sel ? '#fff' : 'var(--btn-text)') + ';border-color:' + (sel ? 'var(--accent2)' : 'var(--btn-border)') + ';">' + label + '</button>';
  }).join('');
}
function selectTradeFilter(g) {
  var fg = document.getElementById('f-group');
  if (fg) fg.value = g;
  buildTradeFilterBar();
  renderIssues();
  if (window._issueFilterState) window._issueFilterState.save();
}
...
function assignIssue(id) {
  ...
  var group = sel.value;
  fetchJSONRetry({ action: ISSUE_CFG.actions.assign, id: id, assignedGroup: group, token: issueToken() || '' })
    .then(function (d) {
      if (d && d.ok === false) {
      ...
      if (it) it.assignedGroup = group;
      renderIssues();
      buildTradeFilterBar();
      openIssue(id);
      uiAlert(group ? ('\u2705 Assigned to ' + tradeGroupLabel(group)) : '\u2705 Assignment cleared');
    })
...
function renderWorkerJobs() {
  ...
  host.innerHTML = rows.map(function (r) {
    ...
    return '<div class="worker-job-card" onclick="openWorkerJob(\'' + r.id + '\')">' + thumb
      + '<div class="worker-job-body"><div class="worker-job-ref">#' + issueRef(r.num) + '</div>'
...
function openWorkerJob(id) {
  ...
  h += '<div class="worker-fix-section"><h3>' + checkIconHtml() + ' Complete this job</h3><p style="font-size:13px;color:var(--text-soft);margin:0 0 10px;">Take a photo of the finished work. Photo is required.</p>';
  h += '<div class="worker-camera-zone" onclick="document.getElementById(\'worker-fix-file\').click()" role="button" tabindex="0" aria-label="Take completion photo">';
  ...
  h += '<strong>Tap to open camera</strong><span>Take a clear photo of the completed fix</span></div>';
  h += '<div class="worker-fix-btns"><button type="button" class="primary" onclick="document.getElementById(\'worker-fix-file\').click()">Take photo</button>';
  h += '<button type="button" onclick="document.getElementById(\'worker-fix-gallery\').click()">From gallery</button></div>';
  ...
  h += '<input type="file" id="worker-fix-gallery" accept="image/*" style="display:none" onchange="handleWorkerFixFile(event)">';
  h += '<img id="worker-fix-preview" class="worker-fix-preview" alt="Completion photo preview">';
...
function processWorkerFixPhoto(file) {
  ...
  compressImage(file, function (url) {
    if (url) {
      _workerFixUrl = url;
      var img = document.getElementById('worker-fix-preview');
      if (img) { img.src = url; img.style.display = 'block'; }
      if (btn) { btn.disabled = false; btn.textContent = 'Mark as fixed'; }
    } else {
      uiAlert('\u274c Photo upload failed. Try again.');
      if (btn) { btn.disabled = true; btn.textContent = 'Mark as fixed'; }
    }
  });
}
function submitWorkerFix(id) {
  if (!_workerFixUrl) { uiAlert('Please take a completion photo first.'); return; }
  var btn = document.getElementById('worker-submit-btn');
  var noteEl = document.getElementById('worker-fix-note');
  var note = noteEl ? noteEl.value.trim() : '';
  if (btn) { btn.disabled = true; btn.textContent = 'Saving\u2026'; }
  fetchJSONRetry({ action: ISSUE_CFG.actions.markFixed, id: id, fixedPhoto: _workerFixUrl, fixNote: note, token: issueToken() || '' })
    .then(function (d) {
      if (d && d.ok === false) {
        if (empireAuthHandleInvalidSession_(d, issueSessionLogoutOpts())) return;
...
function issueSelectToolbarHtml(){ var cnt=selectedIssueCount(); if(!issueSelectMode) return '<div class="issue-select-bar"><span>Tap cards to pick several, then share on WhatsApp.</span><button type="button" onclick="toggleIssueSelectMode()" style="padding:8px 14px;font-size:12px;margin-left:auto;">Select issues</button></div>'; return '<div class="issue-select-bar"><span><strong>'+cnt+'</strong> selected</span><button type="button" onclick="shareSelectedWhatsApp()" style="background:#25D366;color:#fff;border:none;padding:8px 14px;font-size:12px;display:inline-flex;align-items:center;gap:6px;'+(cnt?'':'opacity:0.55;')+'" '+(cnt?'':'disabled')+'>'+whatsappIconHtml()+' Share on WhatsApp</button><button type="button" onclick="clearIssueSelection()" style="padding:8px 14px;font-size:12px;">Clear</button><button type="button" onclick="toggleIssueSelectMode()" style="padding:8px 14px;font-size:12px;margin-left:auto;">Done</button></div>'; }
...
function buildProjectFilterBar(){ var host=document.getElementById('projectFilterBar'); if(!host) return; var cur=(document.getElementById('f-project')||{}).value||''; var list=[['','All'],['ec',projectNames.ec],['es',projectNames.es],['wd',projectNames.wd],['ww',projectNames.ww],['ra',projectNames.ra]]; host.innerHTML=list.map(function(pair){ var p=pair[0], label=pair[1]; var sel=(cur===p); return '<button type="button" onclick="selectProjectFilter(\''+p+'\')" style="background:'+(sel?'var(--accent2)':'var(--btn-bg)')+';color:'+(sel?'#fff':'var(--btn-text)')+';border-color:'+(sel?'var(--accent2)':'var(--btn-border)')+';">'+label+'</button>'; }).join(''); }
function selectProjectFilter(p){ var fp=document.getElementById('f-project'); if(fp) fp.value=p; buildProjectFilterBar(); renderIssues(); if(window._issueFilterState) window._issueFilterState.save(); }
