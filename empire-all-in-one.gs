...
function handleDeleteReport(body) {
  ...
  var rows = sheet.getDataRange().getValues();
  for (var i=1;i<rows.length;i++) {
    if (String(rows[i][0])===String(body.id)) { trashRows_(CLEANING_SHEET,[rows[i]],'delete',body.username); sheet.deleteRow(i+1); return {ok:true,success:true}; }
  }
...
function handleDeleteTaskPhoto(body) {
  ...
  var rows = sheet.getDataRange().getValues();
  for (var i=1;i<rows.length;i++) {
    if (String(rows[i][0])===String(body.id)) { trashRows_(TASK_PHOTOS_SHEET,[rows[i]],'delete',body.username); sheet.deleteRow(i+1); return {ok:true,success:true}; }
  }
